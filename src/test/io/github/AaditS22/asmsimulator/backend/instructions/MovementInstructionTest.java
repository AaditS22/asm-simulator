...
class MovementInstructionTest {
    ...
    @Test
    void popRegisterTest() {
        long rspBefore = state.getRegister("rsp", 8);
        state.getMemory().writeQuad(rspBefore - 8, 777L);
        state.setRegister("rsp", 8, rspBefore - 8);
        Instruction pop = new PopInstruction("popq", Size.QUAD, List.of(
                new RegisterOperand("%rax")
        ));
        pop.execute(state, labelManager);
        assertEquals(777L, state.getRegister("rax", 8));
        assertEquals(rspBefore, state.getRegister("rsp", 8));
    }

    @Test
    void pushPopRoundTripTest() {
        long originalRsp = state.getRegister("rsp", 8);
        state.setRegister("rax", 8, 1234L);
        ...
        assertEquals(1234L, state.getRegister("rbx", 8));
        assertEquals(originalRsp, state.getRegister("rsp", 8));
    }

    // ==================== MOVZB Tests ====================

    @Test
    void movzbRegisterTest() {
        state.setRegister("rax", 8, 0xFFL);
        Instruction movzb = new MovzbInstruction("movzbl", Size.BYTE, List.of(
                new RegisterOperand("%al"),
                new RegisterOperand("%ebx")
        ));
        movzb.execute(state, labelManager);
        assertEquals(0xFFL, state.getRegister("rbx", 4));
    }

    @Test
    void movzbMemoryTest() {
    ...
    void movzwRegisterTest() {
        ...
        assertEquals(0xABCDL, state.getRegister("rbx", 4));
    }

    @Test
    void movzwMemoryTest() {
