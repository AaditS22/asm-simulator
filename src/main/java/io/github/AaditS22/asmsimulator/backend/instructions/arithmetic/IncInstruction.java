...
public class IncInstruction extends Instruction {
    public IncInstruction(String mnemonic, Size size, List<Operand> operands) {
        super(mnemonic, size, operands);
        validateOperandCount(1);

        if (operands.get(0) instanceof ImmediateOperand) {
            throw new IllegalArgumentException("INC cannot be used with an immediate");
        }
    }
    ...
    public void execute(CPUState state, LabelManager labelManager) {
        ...
        state.getFlags().updateIncFlags(value - 1, size.getBytes() * 8);
